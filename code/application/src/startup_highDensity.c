...
//#include "../HardwareAbstractionLayer/inc/timebase.h"
...
// Use this to place vector table in flash. Vector table location has to be set by the NVIC_SetVectorTable function!
/*
	__attribute__ ((section(".isr_vectorsflash")))
...
void Default_Handler(void)
{
  // FIXME infinite loop makes it hard to notice that the controller entered the Default_Handler
  while (1)
  {
  }
}
